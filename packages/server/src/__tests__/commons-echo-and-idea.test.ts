...
const ans = (kind: string, conf: number, testable: number) => ({ kind: { type: 'choice', choice: kind, confidence: conf, probabilities: {} }, testable: { type: 'noul', noul: testable } });
...
it('commons_idea: only concrete, testable Djimitflo changes are harvest candidates', async () => {
  ...
  expect((await run(ans('concrete_djimitflo_change', 0.9, 0.2), 'b'))?.decision).toBe('no'); // no way to check it
  expect((await run(ans('vague_djimitflo_change', 0.9, 0.9), 'c'))?.reason).toBe('kind=vague_djimitflo_change conf=0.90 testable=0.90');
