...
describe('AgentInteractionLedgerService', () => {
  it('projects existing ledgers into a redacted actor-action-target timeline', () => {
    db.prepare(`INSERT INTO messages (id, from_agent_id, to_agent_id, type, payload, created_at)
      ...
      .run(JSON.stringify({ correlation_id: 'goal-1', epistemic_role: 'claim', evidence: ['memory:1'], secret: 'must-not-leak' }));
    db.prepare(`INSERT INTO agent_messages (id, from_agent, to_agent, type, payload_json, timestamp, status)
      VALUES ('durable-1', 'agent-a', 'agent-b', 'question', ?, '2026-09-07T00:00:30Z', 'pending')`)
      .run(JSON.stringify({ correlation_id: 'goal-1', evidence: ['arxiv:1'], secret: 'must-not-leak' }));
    ...
    expect(interactions.find((interaction) => interaction.action === 'message.knowledge_share')).toMatchObject({
      actor: { id: 'agent-a' }, action: 'message.knowledge_share', target: { type: 'agent', id: 'agent-b' }, evidence_refs: ['memory:1'],
    });
    expect(interactions.find((interaction) => interaction.action === 'message.question')).toMatchObject({
      actor: { id: 'agent-a' }, action: 'message.question', source: 'agent_messages', evidence_refs: ['arxiv:1'],
    });
    expect(JSON.stringify(interactions)).not.toContain('must-not-leak');
    expect(JSON.stringify(interactions)).not.toContain('private');
    expect(interactions.find((interaction) => interaction.source === 'messages')).toMatchObject({
      actor: { role: 'claim' }, evidence_refs: ['memory:1'],
    });
  });
  ...
  it('returns an explicit no-activity or observed digest instead of silent success', () => {
    const empty = new AgentInteractionLedgerService(db).digest();
    expect(empty).toMatchObject({
      state: 'NO_ACTIVITY', records: 0, threads: 0, data_quality: 'DEGRADED',
      unavailable_sources: expect.arrayContaining(['reflections', 'dream_opportunities']),
    });
    db.prepare(`INSERT INTO agent_messages (id, from_agent, to_agent, type, payload_json, timestamp, status)
      VALUES ('digest-1', 'agent-a', 'agent-b', 'question', ?, '2026-09-07T00:00:00Z', 'pending')`)
      .run(JSON.stringify({ thread_id: 'thread-1', epistemic_role: 'question', evidence: ['paper:1'] }));
    expect(new AgentInteractionLedgerService(db).digest()).toMatchObject({
      state: 'OBSERVED', records: 1, threads: 1, participating_agents: ['agent-a', 'agent-b'], evidence_linked: 1,
    });
  });
  ...
  it('marks a digest degraded when an evidence source cannot be read', () => {
    ...
    expect(digest).toMatchObject({ data_quality: 'DEGRADED', unavailable_sources: expect.arrayContaining(['messages', 'agent_messages']) });
    sparse.close();
  });
