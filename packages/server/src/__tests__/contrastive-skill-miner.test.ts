...
let db: Database.Database; let miner: ContrastiveSkillMiner;
beforeEach(() => { db = new Database(":memory:"); db.pragma("foreign_keys = ON"); db.exec(schema); runMigrations(db); miner = new ContrastiveSkillMiner(db, { dim: 32 }); });
afterEach(() => { db?.close(); });
describe("G131: Contrastive Skill Miner", () => {
  ...
  it("gets clusters", () => { expect(Array.isArray(miner.getClusters())).toBe(true); });
  it("deduplicates", () => { expect(typeof miner.deduplicate().merged).toBe("number"); });
  it("correct embedding dimension", () => {
