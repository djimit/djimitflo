...
beforeEach(() => {
  ...
  db.exec(schema);
  runMigrations(db);
  miner = new ContrastiveSkillMiner(db, { threshold: 0.5 });
});
...
describe('G131: Contrastive Skill Miner', () => {
  ...
  it('finds similar patterns', () => {
    ...
    const similar = miner.findSimilarPatterns('TypeScript errors', 5);
  ...
  it('creates correct embedding dimension', () => {
    const custom = new ContrastiveSkillMiner(db, { dim: 32 });
    const patterns = custom.mineWithContrast({ id: 'ep-3', topic: 'Test', domains: ['test'], steps: [{ role: 'maker', action: 'test', outcome: 'success' }], success: true, durationMs: 10000 });
