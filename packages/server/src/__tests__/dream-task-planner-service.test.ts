...
describe('DreamTaskPlannerService', () => {
  it('emits one idempotent Paperclip envelope and preserves proposal state', () => {
    ...
    expect(first[0].metadata).toMatchObject({
      ...
      research_protocol: {
        question: expect.any(String), null_hypothesis: expect.any(String),
        falsification_criteria: expect.any(Array), budget: { max_replications: 30, max_failures: 2 },
      },
    });
  ...
  it('exports at most one qualified opportunity per planning cycle by default', () => {
    const db = createTestDb();
    const insert = db.prepare(`INSERT INTO swarm_capabilities
      (id, kind, owner, version, status, risk_ceiling, input_schema_ref, output_schema_ref, eval_score, eval_threshold, removal_strategy, metadata, created_at, updated_at)
      VALUES (?, 'skill', 'test', '1', 'candidate', 'low', '', '', 0.2, 0.8, 'manual_review', '{}', datetime('now'), datetime('now'))`);
    insert.run('cap-one');
    insert.run('cap-two');
    new DreamCycleService(db).runCycle();
    const planner = new DreamTaskPlannerService(db);
    const pending = join(mkdtempSync(join(tmpdir(), 'dream-plan-limit-')), 'pending.jsonl');
