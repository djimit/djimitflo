...
function artifact(overrides: Record<string, unknown> = {}) {
  const value: Record<string, unknown> = {
    schema: 'djimit.openmythos.calibration.v1', run_id: 'run-1', openmythos_commit: 'a'.repeat(40),
    corpus_sha256: corpusHash, corpus_version: '1.0', corpus_schema_version: 1,
    corpus_certification_ready: true, case_result_rows: 30, total_cases: 30, completed_cases: 30,
    calibrated: true, certification_eligible: true, agreement_rate: 0.9,
    lowest_category_agreement_rate: 0.8,
    evidence_sha256: { corpus: corpusHash, run: 'b'.repeat(64), results: 'c'.repeat(64) }, ...overrides,
  };
...
describe('OpenMythosAttestationService', () => {
  ...
  it('imports a hash-verified attestation and updates the operational eligibility path', () => {
