...
describe('SocialLearningCampaignService', () => {
  ...
  it('holds otherwise supported evidence when facilitator provenance is missing or changed', () => {
    const db = createTestDb();
    try {
      const service = new SocialLearningCampaignService(db);
      service.start({ campaign_id: 'social-confirmatory-drift', started_at: '2026-01-01T00:00:00.000Z', days: 7, minimum_pairs: 2, runtime_commit: 'a'.repeat(40), analyzer_commit: 'b'.repeat(40) });
      const candidates = [
        pair(db, 1, '2026-01-02T00:00:00.000Z', 'd'.repeat(40)),
        pair(db, 2, '2026-01-03T00:00:00.000Z', null),
      ];
      candidates.forEach((candidate, index) => db.prepare(`INSERT INTO external_events (id, event_type, source, occurred_at, payload) VALUES (?, 'outcome.observed', 'test', ?, ?)`)
        .run(`drift-outcome-${index}`, `2026-01-0${index + 4}T00:00:00.000Z`, JSON.stringify({ candidate_id: candidate, value: 1, baseline: 0.5, direction: 'increase', causal_status: 'randomized' })));
      const running = service.tick({ observed_at: '2026-01-05T00:00:00.000Z' });
      const complete = service.tick({ observed_at: '2026-01-08T00:00:00.000Z', worldlab: worldlabEvidence('social-confirmatory-drift', running.report_hash) });
      expect(complete).toMatchObject({ status: 'UNDETERMINED', goal_batch: null, runtime_provenance: { status: 'UNDETERMINED', homogeneous: false, pairs_with_provenance: 1, missing_pairs: 1, mismatched_pairs: 1, commit_counts: { ['d'.repeat(40)]: 1 } } });
    } finally { db.close(); }
  });
