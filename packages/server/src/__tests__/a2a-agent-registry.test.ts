...
describe('G125: A2AAgentRegistry', () => {
  it('registers agent', () => {
    const agent = registry.registerAgent({ name: 'Test Agent', endpoint: 'http://localhost:3000', capabilities: ['read', 'write'], memoryScope: ['episodes'], trustLevel: 0.8, status: 'active' });
  ...
  it('gets active agents', () => {
    ...
    expect(registry.getActiveAgents().length).toBe(1);
  });
  ...
  it('finds agents by capability', () => {
    ...
    expect(registry.findAgentsByCapability('search').length).toBe(1);
  });
  ...
  it('updates handoff status', () => {
    ...
    expect(registry.getPendingHandoffs('agent-2').length).toBe(0);
  });

  it('gets pending handoffs', () => {
    registry.createHandoff('agent-1', 'agent-2', {}, []);
    registry.createHandoff('agent-3', 'agent-2', {}, []);
    expect(registry.getPendingHandoffs('agent-2').length).toBe(2);
  });
  ...
  it('updates agent status', () => {
    ...
    expect(registry.getActiveAgents().find(a => a.id === agent.id)).toBeUndefined();
  });
});
