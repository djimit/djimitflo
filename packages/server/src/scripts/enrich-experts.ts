 ...
 *   npx tsx src/scripts/enrich-experts.ts --db <path> [--batch 5] [--spacing-ms 60000] [--backoff-ms 900000] [--once] [--stop-file <path>] [--log <path>]
...
let last = 0;
const source = { async searchAuthorPapers(name: string, limit?: number) {
...
(async () => {
  log({ event: 'start', pending: service.pending(), batch, spacingMs, backoffMs });
  while (true) {
    ...
    const throttled = results.filter((result) => /^ARXIV_HTTP_429|TimeoutError|aborted/i.test(result.reason ?? ''));
