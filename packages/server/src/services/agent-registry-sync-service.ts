...
export class AgentRegistrySyncService {
  ...
  async sync(): Promise<{ synced: number; status: 'DISABLED' | 'OK' | 'ERROR'; error?: string }> {
    ...
    try {
      ...
      // Retired ecosystem components (default: Paperclip, removed 2026-09-21) are not mirrored and are dropped if present.
      const excluded = new Set((process.env.AGENT_REGISTRY_EXCLUDE_FRAMEWORKS ?? 'paperclip').split(',').map((v) => v.trim().toLowerCase()).filter(Boolean));
      const skip = (a: Record<string, unknown>) => excluded.has(String(a.framework ?? '').toLowerCase());
      let synced = 0;
      this.db.transaction(() => {
        for (const a of list as Array<Record<string, unknown>>) {
          if (!a || typeof a.name !== 'string' || !a.name || skip(a)) continue;
        ...
        for (const a of list as Array<Record<string, unknown>>) {
          if (a && typeof a.name === 'string' && skip(a)) this.db.prepare('DELETE FROM registry_agents WHERE name = ?').run(a.name);
        }
      })();
