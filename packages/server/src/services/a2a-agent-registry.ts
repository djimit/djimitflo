...
export class A2AAgentRegistry {
  constructor(private db: Database) {
    ...
    `);
    this.db.exec('CREATE INDEX IF NOT EXISTS idx_a2a_status ON a2a_agent_cards(status)');
    this.db.exec('CREATE INDEX IF NOT EXISTS idx_a2a_trust ON a2a_agent_cards(trust_level DESC)');
  }
  ...
  registerAgent(card: Omit<AgentCard, 'id' | 'createdAt' | 'lastSeen'>): AgentCard {
    ...
    this.db.prepare(`
      INSERT INTO a2a_agent_cards (id, name, endpoint, capabilities_json, memory_scope_json, trust_level, status)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    ...
    return { ...card, id, lastSeen: null, createdAt: now };
  }
  ...
  createHandoff(fromAgent: string, toAgent: string, context: Record<string, unknown>, memoryRefs: string[] = []): HandoffRequest {
    ...
    const now = new Date().toISOString();
    this.db.prepare(`
      INSERT INTO a2a_handoffs (id, from_agent, to_agent, context_json, memory_refs_json)
      VALUES (?, ?, ?, ?, ?)
    `).run(id, fromAgent, toAgent, JSON.stringify(context), JSON.stringify(memoryRefs));
    return { id, fromAgent, toAgent, context, memoryRefs, status: 'pending', createdAt: now };
  }
  ...
  getPendingHandoffs(agentId: string): HandoffRequest[] {
    ...
    return rows.map(r => ({
      ...
      memoryRefs: JSON.parse(r.memory_refs_json) as string[],
      status: r.status as HandoffRequest['status'], createdAt: r.created_at,
    }));
  }

  private rowToCard(row: { id: string; name: string; endpoint: string; capabilities_json: string; memory_scope_json: string; trust_level: number; status: string; last_seen: string | null; created_at: string }): AgentCard {
    return {
      id: row.id, name: row.name, endpoint: row.endpoint,
      capabilities: JSON.parse(row.capabilities_json) as string[],
      memoryScope: JSON.parse(row.memory_scope_json) as string[],
      trustLevel: row.trust_level, status: row.status as AgentCard['status'],
