...
export class A2AAgentRegistry {
  constructor(private db: Database) {
    this.db.exec(`CREATE TABLE IF NOT EXISTS a2a_agent_cards (id TEXT PRIMARY KEY, name TEXT NOT NULL, endpoint TEXT NOT NULL, capabilities_json TEXT NOT NULL DEFAULT '[]', memory_scope_json TEXT NOT NULL DEFAULT '[]', trust_level REAL NOT NULL DEFAULT 0.5, status TEXT NOT NULL DEFAULT 'inactive', last_seen TEXT, created_at TEXT NOT NULL DEFAULT (datetime('now')))`);
    this.db.exec(`CREATE TABLE IF NOT EXISTS a2a_handoffs (id TEXT PRIMARY KEY, from_agent TEXT NOT NULL, to_agent TEXT NOT NULL, context_json TEXT NOT NULL DEFAULT '{}', memory_refs_json TEXT NOT NULL DEFAULT '[]', status TEXT NOT NULL DEFAULT 'pending', created_at TEXT NOT NULL DEFAULT (datetime('now')))`);
  }
  ...
  getActiveAgents(): AgentCard[] {
    ...
    return rows.map(r => ({ id: r.id, name: r.name, endpoint: r.endpoint, capabilities: JSON.parse(r.capabilities_json), memoryScope: JSON.parse(r.memory_scope_json), trustLevel: r.trust_level, status: r.status as AgentCard['status'], lastSeen: r.last_seen, createdAt: r.created_at }));
  }
  ...
  getPendingHandoffs(agentId: string): HandoffRequest[] {
    ...
    return rows.map(r => ({ id: r.id, fromAgent: r.from_agent, toAgent: r.to_agent, context: JSON.parse(r.context_json), memoryRefs: JSON.parse(r.memory_refs_json), status: r.status as HandoffRequest['status'], createdAt: r.created_at }));
  }
}
