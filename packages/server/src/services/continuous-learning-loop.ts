...
export class ContinuousLearningLoop {
  ...
  constructor(private db: Database, options: { intervalMs?: number } = {}) {
    ...
    this.selfModel = new SelfModelService(db);
  ...
  async runCycle(): Promise<LearningCycleResult> {
    ...
    const id = randomUUID();
    const previous = this.getLastCycle();
    const since = previous?.sourceWatermark || previous?.timestamp || '1970-01-01T00:00:00.000Z';
    const pendingEpisodes = this.collectPendingEpisodes(since, cycleStartedAt);
    ...
    for (const episode of pendingEpisodes) {
      const runId = episode.id.replace(/^episode-/, '');
      this.reflections.reflectOnRun(runId);
    ...
    const result: LearningCycleResult = { id, timestamp: new Date().toISOString(), sourceWatermark: cycleStartedAt, episodesIngested, reflectionsGenerated, patternsDetected: patternReport.recurringPatterns.length, goalsGenerated: generatedGoals.total, durationMs: Date.now() - start };
  ...
  private collectPendingEpisodes(since: string, until: string): Array<{ id: string; type: string; content: string; source: string; timestamp: string }> {
    const episodes: Array<{ id: string; type: string; content: string; source: string; timestamp: string }> = [];
    try {
      const runs = this.db.prepare(`
        SELECT id, loop_name, status, created_at
        FROM loop_runs
        WHERE status = 'completed' AND julianday(created_at) > julianday(?) AND julianday(created_at) <= julianday(?)
        ORDER BY created_at ASC LIMIT 10
      `).all(since, until) as Array<{ id: string; loop_name: string; status: string; created_at: string }>;
