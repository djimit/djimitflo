...
export class SkillEvolutionGym {
  ...
  evaluateSkill(skillId: string, metrics: Record<string, number>): GymEvaluation {
    ...
    this.db.prepare('INSERT INTO gym_evaluations (id, skill_id, score, metrics_json) VALUES (?, ?, ?, ?)')
      .run(id, skillId, score, JSON.stringify(metrics));

    return { id, skillId, score, metrics, timestamp: now };
  }
  ...
  exploreDomain(domain: string): ExplorationResult {
    ...
    if (domainPatterns.length > 0) {
      recommendation = `Found ${domainPatterns.length} patterns — top: "${domainPatterns[0].name}" (evidence: ${domainPatterns[0].evidence})`;
    } else if (promptPatterns.length > 0) {
      recommendation = `No skill patterns, but ${promptPatterns.length} prompt patterns available for "${domain}"`;
    }

    return {
      domain,
      patternsFound: domainPatterns.length,
      topPattern: domainPatterns[0]?.name ?? null,
      recommendation,
    };
  }
  ...
  ingestEpisode(episode: SwarmEpisode): { patterns: SkillPattern[]; evaluation: GymEvaluation | null } {
    ...
    if (patterns.length > 0) {
      evaluation = this.evaluateSkill(patterns[0].id, {
        ...
        duration: Math.min(1, episode.durationMs / 60000),
  ...
  getLeaderboard(limit: number = 10): Array<{ skillId: string; avgScore: number; evaluations: number }> {
    try {
      const rows = this.db.prepare(`
        SELECT skill_id, AVG(score) as avg_score, COUNT(*) as count
        FROM gym_evaluations GROUP BY skill_id ORDER BY avg_score DESC LIMIT ?
      `).all(limit) as Array<{ skill_id: string; avg_score: number; count: number }>;
  ...
  getStats(): { totalEvaluations: number; totalPatterns: number; domains: string[] } {
    ...
    try {
      const row = this.db.prepare('SELECT COUNT(*) as c FROM gym_evaluations').get() as { c: number };
      totalEvaluations = row.c;
    } catch { /* table may not exist */ }

    const patterns = this.miner.getPatterns(1, 100);
    const domains = [...new Set(patterns.flatMap(p => p.domains))];

    return { totalEvaluations, totalPatterns: patterns.length, domains };
  }
}
