...
export interface AgentInteractionDigest {
  generated_at: string;
  records: number;
  threads: number;
  participating_agents: string[];
  evidence_linked: number;
  epistemic_roles: Record<string, number>;
  sources: Record<string, number>;
  state: 'OBSERVED' | 'NO_ACTIVITY';
  data_quality: 'COMPLETE' | 'DEGRADED';
  unavailable_sources: string[];
}

export class AgentInteractionLedgerService {
  private readonly unavailableSources = new Set<string>();
  ...
  list(filter: AgentInteractionFilter = {}): AgentInteractionRecord[] {
    ...
    const interactions = [
      ...
      ...this.durableAgentMessages(fetchLimit),
  ...
  digest(limit = 500): AgentInteractionDigest {
    const records = this.list({ limit });
    const threads = new Set(records.map((item) => item.correlation_id || `unattributed:${item.id}`));
    const agents = new Set(records.flatMap((item) => [item.actor.id, item.target?.type === 'agent' ? item.target.id : null].filter((value): value is string => Boolean(value))));
    ...
    return {
      ...
      participating_agents: [...agents].sort(),
      evidence_linked: records.filter((item) => item.evidence_refs.length > 0).length,
      epistemic_roles: count(records.map((item) => item.actor.role).filter((value): value is string => Boolean(value))),
      sources: count(records.map((item) => item.source)),
      state: records.length ? 'OBSERVED' : 'NO_ACTIVITY',
      data_quality: this.unavailableSources.size ? 'DEGRADED' : 'COMPLETE',
      unavailable_sources: [...this.unavailableSources].sort(),
    };
  }

  dataQuality(): Pick<AgentInteractionDigest, 'data_quality' | 'unavailable_sources'> {
    return {
      data_quality: this.unavailableSources.size ? 'DEGRADED' : 'COMPLETE',
      unavailable_sources: [...this.unavailableSources].sort(),
    };
    ...
    return this.query(`SELECT id, from_agent_id, to_agent_id, type, payload, read_at, created_at FROM messages ORDER BY created_at DESC LIMIT ?`, limit)
      .map((row) => {
        ...
        return this.record({
          ...
          actorRole: this.string(metadata.epistemic_role) || this.string(metadata.actor_role), runtime: this.string(metadata.runtime), model: this.string(metadata.model_id),
          ...
          evidenceRefs: [...this.stringArray(metadata.evidence), ...this.stringArray(metadata.evidence_refs)], effectScope: this.scope(metadata.effect_scope, 'isolated'),
          source: 'messages', summary: `${row.from_agent_id} sent ${row.type} to ${row.to_agent_id}`,
        });
      });
  }

  private durableAgentMessages(limit: number): AgentInteractionRecord[] {
    return this.query(`SELECT id, from_agent, to_agent, type, priority, payload_json, timestamp, status FROM agent_messages ORDER BY timestamp DESC LIMIT ?`, limit)
      .map((row) => {
        ...
        return this.record({
          ...
          summary: social
            ? this.string(params.board_summary) || this.string(payload.context) || `${row.from_agent} sent ${action} to ${row.to_agent}`
  ...
  private query(sql: string, limit: number): any[] {
    try { return this.db.prepare(sql).all(limit) as any[]; } catch {
      const source = sql.match(/\bFROM\s+([a-z_][a-z0-9_]*)/i)?.[1];
      if (source) this.unavailableSources.add(source);
      return [];
    }
  }
