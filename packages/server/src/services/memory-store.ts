...
export class SqliteMemoryStore implements MemoryStore {
  constructor(private db: Database) {
    ...
    this.db.exec(`
      CREATE TABLE IF NOT EXISTS memory_relations (
        ...
        created_at TEXT NOT NULL DEFAULT (datetime('now')),
        FOREIGN KEY (from_id) REFERENCES central_memories(id),
        FOREIGN KEY (to_id) REFERENCES central_memories(id)
      )
    `);
    this.db.exec('CREATE INDEX IF NOT EXISTS idx_cm_type ON central_memories(type)');
    this.db.exec('CREATE INDEX IF NOT EXISTS idx_cm_source ON central_memories(source)');
    this.db.exec('CREATE INDEX IF NOT EXISTS idx_cm_confidence ON central_memories(confidence)');
    this.db.exec('CREATE INDEX IF NOT EXISTS idx_mr_from ON memory_relations(from_id)');
    this.db.exec('CREATE INDEX IF NOT EXISTS idx_mr_to ON memory_relations(to_id)');
  }
  ...
  store(record: Omit<MemoryRecord, 'id' | 'createdAt'>): MemoryRecord {
    ...
    this.db.prepare(`
      INSERT INTO central_memories (id, type, content, source, confidence, metadata_json)
      VALUES (?, ?, ?, ?, ?, ?)
    `).run(id, record.type, record.content, record.source, record.confidence, JSON.stringify(record.metadata));
  ...
  relate(fromId: string, toId: string, relationType: string, strength: number): MemoryRelation {
    const id = randomUUID();
    this.db.prepare(`
      INSERT INTO memory_relations (id, from_id, to_id, relation_type, strength)
      VALUES (?, ?, ?, ?, ?)
    `).run(id, fromId, toId, relationType, strength);
    return { id, fromId, toId, relationType, strength };
  }

  getRelations(id: string): MemoryRelation[] {
    const rows = this.db.prepare(
      'SELECT * FROM memory_relations WHERE from_id = ? OR to_id = ? ORDER BY strength DESC'
    ).all(id, id) as RelationRow[];
    return rows.map(r => ({ id: r.id, fromId: r.from_id, toId: r.to_id, relationType: r.relation_type, strength: r.strength }));
  }
  ...
  project(id: string, depth: number): MemoryRecord[] {
    ...
    const visit = (currentId: string, currentDepth: number) => {
      if (currentDepth > depth || visited.has(currentId)) return;
      visited.add(currentId);

      const record = this.retrieve(currentId);
...
export class InMemoryMemoryStore implements MemoryStore {
  ...
  project(id: string, depth: number): MemoryRecord[] {
    ...
    const visit = (currentId: string, currentDepth: number) => {
      if (currentDepth > depth || visited.has(currentId)) return;
      visited.add(currentId);
      const record = this.memories.get(currentId);
      if (record) results.push(record);
      if (currentDepth < depth) {
        for (const rel of this.relations.values()) {
          if (rel.fromId === currentId) visit(rel.toId, currentDepth + 1);
          if (rel.toId === currentId) visit(rel.fromId, currentDepth + 1);
        }
      }
    };
