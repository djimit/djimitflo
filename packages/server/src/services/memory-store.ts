...
export class SqliteMemoryStore implements MemoryStore {
  ...
  store(record: Omit<MemoryRecord, 'id' | 'createdAt'>): MemoryRecord {
    const id = randomUUID();
    this.db.prepare('INSERT INTO central_memories (id, type, content, source, confidence, metadata_json) VALUES (?, ?, ?, ?, ?, ?)')
  ...
  relate(fromId: string, toId: string, relationType: string, strength: number): MemoryRelation {
    const id = randomUUID();
    this.db.prepare('INSERT INTO memory_relations (id, from_id, to_id, relation_type, strength) VALUES (?, ?, ?, ?, ?)')
      .run(id, fromId, toId, relationType, strength);
    return { id, fromId, toId, relationType, strength };
  }

  getRelations(id: string): MemoryRelation[] {
    return (this.db.prepare('SELECT * FROM memory_relations WHERE from_id = ? OR to_id = ? ORDER BY strength DESC').all(id, id) as RelationRow[])
      .map(r => ({ id: r.id, fromId: r.from_id, toId: r.to_id, relationType: r.relation_type, strength: r.strength }));
  }
  ...
  project(id: string, depth: number): MemoryRecord[] {
    ...
    const visit = (curr: string, d: number) => {
      ...
      if (d < depth) {
        for (const rel of this.getRelations(curr)) {
          visit(rel.fromId === curr ? rel.toId : rel.fromId, d + 1);
        }
      }
    };
    visit(id, 0);
