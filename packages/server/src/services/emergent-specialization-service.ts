...
export class EmergentSpecializationService {
  private emergingThreshold = 3;
  private establishedThreshold = 0.8;
  private pruneThreshold = 0.2;
  private pruneMinRuns = 5;
  ...
  constructor(private db: Database) {
    ...
    `);
    this.db.exec('CREATE INDEX IF NOT EXISTS idx_spec_agent ON agent_specializations(agent_id)');
    this.db.exec('CREATE INDEX IF NOT EXISTS idx_spec_status ON agent_specializations(status)');
  }

  recordPerformance(agentId: string, domain: string, subDomain: string, success: boolean): void {
    const existing = this.db.prepare(
      'SELECT n_runs, success_rate FROM agent_specializations WHERE agent_id = ? AND domain = ? AND sub_domain = ?'
    ).get(agentId, domain, subDomain) as { n_runs: number; success_rate: number } | undefined;
    ...
    if (newRuns >= this.emergingThreshold && newSuccessRate >= this.establishedThreshold) {
      status = 'established';
    } else if (newRuns >= this.pruneMinRuns && newSuccessRate < this.pruneThreshold) {
      status = 'pruned';
    }

    const id = randomUUID();
    this.db.prepare(`
      INSERT OR REPLACE INTO agent_specializations (id, agent_id, domain, sub_domain, n_runs, success_rate, status, last_activity)
      VALUES (?, ?, ?, ?, ?, ?, ?, datetime('now'))
    `).run(id, agentId, domain, subDomain, newRuns, newSuccessRate, status);
  }
  ...
  getSpecializations(agentId?: string, status?: string): SpecializationRecord[] {
    ...
    const rows = this.db.prepare(query).all(...params) as SpecRow[];
    return rows.map(r => ({
      agentId: r.agent_id,
      domain: r.domain,
      ...
      status: r.status as SpecializationRecord['status'],
  ...
  getPrunedSpecializations(): SpecializationRecord[] {
    return this.getSpecializations(undefined, 'pruned');
  }
