...
export class SkillPatternMiner {
  constructor(private db: Database) {
    ...
    `);
    this.db.exec('CREATE INDEX IF NOT EXISTS idx_sp_evidence ON skill_patterns(evidence DESC)');
  }

  mineFromEpisode(episode: SwarmEpisode): SkillPattern[] {
    const patterns: SkillPattern[] = [];

    const actionSequence = episode.steps
      .filter(s => s.outcome === 'success')
      .map(s => s.action)
      .join(' → ');

    if (actionSequence.length > 0 && episode.success) {
      patterns.push(this.createPattern({
        name: `Pattern: ${episode.topic}`,
        description: `Successful action sequence for ${episode.topic}`,
        steps: episode.steps.filter(s => s.outcome === 'success').map(s => s.action),
  ...
  mineFromEpisodes(episodes: SwarmEpisode[]): SkillPattern[] {
    ...
    for (const episode of episodes) {
      allPatterns.push(...this.mineFromEpisode(episode));
    }
    return allPatterns;
  }
  ...
  getPatterns(minEvidence: number = 1, limit: number = 20): SkillPattern[] {
    ...
    return rows.map(r => ({
      ...
      evidence: r.evidence, domains: JSON.parse(r.domains_json) as string[],
  ...
  private createPattern(input: { name: string; description: string; steps: string[]; domains: string[]; success: boolean }): SkillPattern {
    ...
    this.db.prepare(`
      INSERT INTO skill_patterns (id, name, description, steps_json, evidence, domains_json, success_rate)
      VALUES (?, ?, ?, ?, 1, ?, ?)
    `).run(id, input.name, input.description, JSON.stringify(input.steps), JSON.stringify(input.domains), input.success ? 1 : 0);

    return { id, name: input.name, description: input.description, steps: input.steps, evidence: 1, domains: input.domains, successRate: input.success ? 1 : 0, createdAt: now };
  }
}
