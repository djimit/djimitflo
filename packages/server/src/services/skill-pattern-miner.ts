...
export class SkillPatternMiner {
  constructor(private db: Database) {
    this.db.exec(`CREATE TABLE IF NOT EXISTS skill_patterns (id TEXT PRIMARY KEY, name TEXT NOT NULL, description TEXT NOT NULL, steps_json TEXT NOT NULL DEFAULT '[]', evidence INTEGER NOT NULL DEFAULT 0, domains_json TEXT NOT NULL DEFAULT '[]', success_rate REAL NOT NULL DEFAULT 0, created_at TEXT NOT NULL DEFAULT (datetime('now')), updated_at TEXT NOT NULL DEFAULT (datetime('now')))`);
  }
  ...
  getPatterns(minEvidence: number = 1, limit: number = 20): SkillPattern[] {
    ...
    return rows.map(r => ({ id: r.id, name: r.name, description: r.description, steps: JSON.parse(r.steps_json), evidence: r.evidence, domains: JSON.parse(r.domains_json), successRate: r.success_rate, createdAt: r.created_at }));
  }
