...
export class PromptPatternRegistry {
  constructor(private db: Database) {
    ...
    `);
    this.db.exec('CREATE INDEX IF NOT EXISTS idx_pp_domain ON prompt_patterns(domain)');
    this.db.exec('CREATE INDEX IF NOT EXISTS idx_pp_success ON prompt_patterns(success_count DESC)');
  }
  ...
  register(name: string, template: string, domain: string = 'general'): PromptPattern {
    ...
    this.db.prepare(`
      INSERT INTO prompt_patterns (id, name, template, domain)
      VALUES (?, ?, ?, ?)
    `).run(id, name, template, domain);
    return { id, name, template, domain, successCount: 0, failCount: 0, lastEvaluatedAt: null, createdAt: now };
  }
  ...
  evaluate(patternId: string, beforeScore: number, afterScore: number): PromptEvaluation {
  ...
  getPatternsForDomain(domain: string, limit: number = 10): PromptPattern[] {
    ...
    return rows.map(r => ({
      id: r.id, name: r.name, template: r.template, domain: r.domain,
      successCount: r.success_count, failCount: r.fail_count,
      lastEvaluatedAt: r.last_evaluated_at, createdAt: r.created_at,
    }));
  }

  getBestPattern(domain: string): PromptPattern | null {
    const patterns = this.getPatternsForDomain(domain, 1);
    return patterns[0] ?? null;
  }

  getAllPatterns(limit: number = 50): PromptPattern[] {
    const rows = this.db.prepare('SELECT * FROM prompt_patterns ORDER BY success_count DESC LIMIT ?').all(limit) as Array<{
      id: string; name: string; template: string; domain: string; success_count: number; fail_count: number; last_evaluated_at: string | null; created_at: string;
    }>;
    return rows.map(r => ({
      id: r.id, name: r.name, template: r.template, domain: r.domain,
