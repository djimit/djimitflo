...
export function createOrganizationRoutes(db: Database, requireAuthMiddleware: any, authService: any, auditService?: any): Router {
   ...
   * Available organizations follow current membership, not the selected context.
   */
  router.get('/', orgLimiter, requireAuthMiddleware, (req, res) => {
    const userOrgId = authService.findUserById(req.user!.sub)?.organization_id || 'default';
  ...
  router.post('/switch', orgLimiter, requireAuthMiddleware, async (req, res) => {
    ...
    if (organization_id !== undefined && (typeof organization_id !== 'string' || !organization_id.trim())) {
    ...
    if (organization_id !== undefined && (user.organization_id ?? 'default') !== organization_id && organization_id !== 'default') {
    ...
    const token = authService.generateToken(targetUser, req.user!.sid);
    if (auditService && typeof auditService.record === 'function') {
      auditService.record({
        ...
        resource_type: 'user',
        resource_id: user.id,
        metadata: { from: req.user!.organization_id || 'default', to: targetUser.organization_id },
      });
    }
