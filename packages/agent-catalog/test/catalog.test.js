...
import { AgentCatalog, parseAgentMarkdown, normalizeAgent, validateSchema, scanInjection, overlapScore, compile } from '../src';
const FIX = (f) => readFileSync(join(process.cwd(), 'fixtures/agents', f), 'utf8');
describe('parser', () => {
...
describe('activation', () => {
    ...
    it('blocks activation with no evaluation record at all', () => {
        const cat = new AgentCatalog();
        cat.db.upsertProfile({ id: 'x/y', name: 'Y', division: 'd', source_repo: 'r', source_path: 'p', version_hash: 'h', persona: '', mission: '', rules: [], workflows: [], deliverables: [], success_metrics: [], memory_policy: '', tools_required: [], runtime_targets: [], risk_profile: { level: 'low', injection_score: 0, overlap_score: 0, flags: [] }, evaluation_status: 'pending', activation_status: 'draft' });
...
describe('compile', () => {
    ...
    it('stub targets flagged', () => {
        const prof = normalizeAgent(parseAgentMarkdown(FIX('secure-coder.md'), { sourceRepo: 'r', sourcePath: 'engineering/a.md' }));
        for (const t of ['claude-code', 'cursor', 'gemini-cli'])
            expect(compile(prof, t).stub).toBe(true);
    });
});
