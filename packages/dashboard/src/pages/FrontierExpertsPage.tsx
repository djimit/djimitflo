        ...
        <section className="rounded-xl border border-border bg-background-secondary p-4">
          ...
          {detail && (
            <div className="mt-3 space-y-4 text-xs">
              ...
              {!!detail.peer_reviews?.length && <section className="space-y-2"><h3 className="font-semibold text-foreground">Onderlinge bronreviews</h3><p className="text-foreground-secondary">AI-beoordelingen vanuit een andere onderzoekslens; geen persoonlijke endorsements of automatische goedkeuring.</p>{detail.peer_reviews.map((review) => <details key={review.audit_id} className="rounded-lg border border-border p-2"><summary className="cursor-pointer">{time(review.created_at)} · {review.runtime} · profiel v{review.expert_version}</summary><Link to={`/frontier-experts/${review.reviewer_id}`} className="underline">Onderzoekslens bekijken</Link><ul className="mt-2 space-y-2">{review.checks.map((check) => <li key={check.capability_id}><strong>{check.capability_id}: {check.decision}</strong><p>{check.rationale}</p><p className="break-all text-foreground-tertiary">Bronnen: {[...check.evidence_refs, ...check.reviewer_evidence_refs].join(', ') || 'Ontbreken'}</p></li>)}</ul></details>)}</section>}
