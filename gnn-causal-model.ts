...
export interface GraphNode { id: string; features: number[]; nodeType: string; label: string; }
export interface GraphEdge { id: string; from: string; to: string; relation: string; weight: number; }
export interface CausalPrediction { intervention: string; predictedOutcome: number; confidence: number; path: string[]; }
export class GNNCausalModel {
  private hidden: number; private layers: number;
  constructor(private db: Database, options: { hidden?: number; layers?: number } = {}) {
    this.hidden = options.hidden ?? 32; this.layers = options.layers ?? 2;
  ...
  addNode(node: GraphNode): void { const emb = this.initEmb(node.features); this.db.prepare("INSERT OR REPLACE INTO gnn_nodes (id, features_json, node_type, label, embedding_json) VALUES (?, ?, ?, ?, ?)").run(node.id, JSON.stringify(node.features), node.nodeType, node.label, JSON.stringify(emb)); }
  addEdge(edge: GraphEdge): void { this.db.prepare("INSERT OR REPLACE INTO gnn_edges (id, from_node, to_node, relation, weight) VALUES (?, ?, ?, ?, ?)").run(edge.id, edge.from, edge.to, edge.relation, edge.weight); }
  predict(interventionId: string): CausalPrediction {
    ...
    let total = 0, weight = 0; const path: string[] = [];
    for (const o of outcomes) { const dist = this.distance(interventionId, o.id); const w = 1 / (1 + dist); total += o.features[0] * w; weight += w; path.push(o.label); }
  ...
  learn(features: Record<string, string>, outcome: number): void {
    const ctxId = randomUUID(); const outId = randomUUID();
  ...
  private initEmb(features: number[]): number[] { const emb = new Array(this.hidden).fill(0); for (let i = 0; i < Math.min(features.length, this.hidden); i++) emb[i] = features[i]; const norm = Math.sqrt(emb.reduce((s, v) => s + v * v, 0)); if (norm > 0) for (let i = 0; i < emb.length; i++) emb[i] /= norm; return emb; }
  private getReachable(startId: string, maxDepth: number): GraphNode[] {
    const visited = new Set<string>(); const result: GraphNode[] = [];
    const queue: Array<{ id: string; depth: number }> = [{ id: startId, depth: 0 }];
    while (queue.length > 0) { const { id, depth } = queue.shift()!; if (visited.has(id) || depth > maxDepth) continue; visited.add(id);
      const node = this.db.prepare("SELECT * FROM gnn_nodes WHERE id = ?").get(id) as any;
      if (node) { result.push({ id: node.id, features: JSON.parse(node.features_json), nodeType: node.node_type, label: node.label });
  ...
  private distance(from: string, to: string): number {
    const visited = new Set<string>(); const queue: Array<{ id: string; dist: number }> = [{ id: from, dist: 0 }];
    while (queue.length > 0) { const { id, dist } = queue.shift()!; if (id === to) return dist; if (visited.has(id)) continue; visited.add(id);
      const edges = this.db.prepare("SELECT to_node FROM gnn_edges WHERE from_node = ?").all(id) as Array<{ to_node: string }>;
      for (const edge of edges) if (!visited.has(edge.to_node)) queue.push({ id: edge.to_node, dist: dist + 1 }); }
    return Infinity;
  }
  private hash(value: string): number { let h = 0; for (let i = 0; i < value.length; i++) { h = ((h << 5) - h) + value.charCodeAt(i); h |= 0; } return (h % 1000) / 1000; }
}
