...
def run_hermes(prompt):
    result = subprocess.run([os.environ.get('HERMES_BIN', 'hermes'), 'chat', '--query-file', '-', '--oneshot', '--quiet', '--toolsets', '', '--max-turns', '1', '--run-budget', '90', '--source', 'djimitflo-social'], input=prompt, text=True, capture_output=True, cwd=os.environ.get('HERMES_HOME', os.path.expanduser('~/.hermes')), env={**os.environ, 'HOME': os.path.expanduser('~')}, timeout=150)
    if result.returncode: raise RuntimeError(f'Hermes failed with exit {result.returncode}: {result.stderr[-500:]}')
    sessions = re.findall(r'session_id:\s*([^\s]+)', result.stdout)
    return result.stdout, sessions[-1] if sessions else '', {}
...
def run_deerflow(prompt):
    ...
    result = subprocess.run(['docker', 'exec', '-i', os.environ.get('SOCIAL_DEERFLOW_CONTAINER', 'deer-flow-gateway'), 'python', '-c', runner], input=prompt, text=True, capture_output=True, timeout=210)
