...
# a hung build (0% CPU for 17-20 min, 2026-09-25 and 26) blocked every deploy until killed by hand: bound it
(cd "runtime-source-$SHORT" && timeout "${DEPLOY_BUILD_TIMEOUT_S:-1200}" docker build --build-arg VCS_REF="$SHA" --build-arg BUILD_SOURCE=deploy-vps.sh --build-arg BUILD_TIME="$(date -u +%Y-%m-%dT%H:%M:%SZ)" -t "djimitflo:main-$SHORT" . 2>&1 | tail -n 2)
