...
export function verifyIdentity({ health, version, provenance, integrity, commit, dirty, instanceId }) {
  ...
  return Boolean(!dirty && /^[a-f0-9]{40}$/.test(commit)
    ...
    && instanceId && database?.instance_id === instanceId);
}
...
const identityVerified = verifyIdentity({ health, version, provenance, integrity, commit, dirty: Boolean(dirtyState), instanceId: instance.ok ? instance.output : '' });
const report = {
  schema_version: 1,
  generated_at: new Date().toISOString(),
  status: identityVerified ? 'pass' : health.ok ? 'blocked' : 'fail',
  intended: { commit, dirty: Boolean(dirtyState), dirty_state_sha256: createHash('sha256').update(dirtyState).digest('hex'), database: dbPath, database_instance_id: instance.ok ? instance.output : null },
