...
const results = [];
function runTest(name, category, fn) {
    const start = Date.now();
    try {
        const result = fn();
        const duration = Date.now() - start;
        const testResult = { name, category, passed: result.passed, durationMs: duration, details: result.details, metrics: result.metrics };
        ...
        const duration = Date.now() - start;
        const err = error;
        const testResult = { name, category, passed: false, durationMs: duration, details: err.message, metrics: {} };
        results.push(testResult);
...
console.log('');
if (score >= 80)
    console.log('EXCELLENT - DjimFlo performs well on realistic tasks');
else if (score >= 60)
    console.log('GOOD - DjimFlo handles most tasks correctly');
else
    console.log('NEEDS IMPROVEMENT - Some tasks failed');
fs.rmSync(WORKSPACE, { recursive: true, force: true });
process.exit(score >= 60 ? 0 : 1);
//# sourceMappingURL=realistic-test-suite.js.map