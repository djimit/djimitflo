...
function runCommand(cmd, cwd, timeoutMs = 30000) {
    try {
...
function createTask(name, category, fn) {
    const start = Date.now();
    try {
        const result = fn();
        const duration = Date.now() - start;
        const taskResult = { name, category, passed: result.passed, durationMs: duration, output: result.output, errors: result.errors };
        ...
        const duration = Date.now() - start;
        const err = error;
        const taskResult = { name, category, passed: false, durationMs: duration, output: '', errors: [err.message] };
        results.push(taskResult);
...
//# sourceMappingURL=djimflo-self-benchmark.js.map