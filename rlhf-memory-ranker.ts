...
export class RLHFMemoryRanker {
  private clip: number; private window: number;
  constructor(private db: Database, options: { clip?: number; window?: number } = {}) {
    this.clip = options.clip ?? 0.2; this.window = options.window ?? 20;
  ...
  recordReward(memoryId: string, runId: string, outcome: string, utility: number): void {
    this.db.prepare("INSERT INTO memory_rewards (id, memory_id, loop_run_id, outcome, utility_score) VALUES (?, ?, ?, ?, ?)").run(randomUUID(), memoryId, runId, outcome, utility);
    this.updatePolicy(memoryId);
  }
  rank(memories: MemoryRecord[]): RankedMemory[] {
    const ranked = memories.map((r, i) => { const p = this.getPolicy(r.id); return { record: r, reward: p?.reward ?? 0.5, rank: i + 1 }; });
    ranked.sort((a, b) => b.reward - a.reward); ranked.forEach((r, i) => { r.rank = i + 1; }); return ranked;
  }
  getTop(limit: number = 10): RankedMemory[] {
    ...
    return this.rank(rows.map(m => ({ id: m.id, type: m.type, content: m.content, source: m.source, confidence: m.confidence, metadata: JSON.parse(m.metadata_json), createdAt: m.created_at })));
  }
  prune(threshold: number = 0.2): number {
    ...
    let n = 0; for (const { memory_id } of low) { try { this.db.prepare("DELETE FROM memory_rewards WHERE memory_id = ?").run(memory_id); this.db.prepare("DELETE FROM memory_policy WHERE memory_id = ?").run(memory_id); n++; } catch {} }
    return n;
  }
  stats(): { avgReward: number; totalUpdates: number } {
    const r = this.db.prepare("SELECT AVG(reward) as avg, SUM(update_count) as total FROM memory_policy").get() as any;
    return { avgReward: r.avg ?? 0.5, totalUpdates: r.total ?? 0 };
  }
  private updatePolicy(memoryId: string): void {
    ...
    const existing = this.getPolicy(memoryId); const oldReward = existing?.reward ?? 0.5;
    const advantage = avg - oldReward; const clipped = Math.max(-this.clip, Math.min(this.clip, advantage));
  ...
  private getPolicy(memoryId: string): { reward: number; advantage: number; update_count: number } | null {
    const r = this.db.prepare("SELECT * FROM memory_policy WHERE memory_id = ?").get(memoryId) as any;
    return r ? { reward: r.reward, advantage: r.advantage, update_count: r.update_count } : null;
  }
}
