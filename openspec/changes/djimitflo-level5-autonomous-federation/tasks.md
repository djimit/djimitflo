...
- [x] T19.1 Evolve `LoopDaemon` to `ParallelLoopDaemon`: process multiple goals
...
- [x] T19.2 Track active goals in a `Set<string>` (in-memory) + persist active goal
      IDs in `system_state` so they survive restarts (G10 crash recovery applies).
- [x] T19.3 The AIMD controller is the global concurrency gate — total concurrent
...
- [x] T20.1 Add `HelpRequest` + `HelpResponse` types to the knowledge bus.
- [x] T20.2 `NegotiationCoordinator` service: receives `help_request` → checks
      capability + capacity → spawns a nested specialist (via `NestedSpawnService`) →
      emits `help_response`.
- [x] T20.3 A maker can emit a `help_request` via `knowledgeBus.publish` when it
      encounters a harder-than-expected problem. The request includes the needed
      capability + urgency.
- [x] T20.4 Cycle guard: the coordinator rejects help_requests that would create a
...
- [x] T21.1 `decomposeGoalToDAG(goalId)`: uses the runtime (codex, headless, sandboxed)
      to parse the goal's objective into steps → matches each step to a capability →
      builds a DAG with dependencies.
- [x] T21.2 The scheduler executes the DAG layer by layer (dependencies first, bounded
      concurrency per layer via the AIMD controller).
- [x] T21.3 Backward compatibility: if decomposition fails (no runtime, parse error),
      fall back to `decomposeGoal` (predefined loop contracts).
- [x] T21.4 The decomposition result is stored as `goal.metadata.dag` for observability
      and resume (G10).
...
- [x] T22.1 `POST /api/goals/:id/pause` — pause the goal (stop accepting new findings,
      drain in-flight leases via G9 `drainRuntimeLeases`).
- [x] T22.2 `POST /api/goals/:id/resume` — resume a paused goal (re-queue pending
      findings via G10 `resumeInterruptedRun`).
- [x] T22.3 `POST /api/goals/:id/inject` — inject a claim into the semantic store
      (knowledge injection by the operator).
- [x] T22.4 `POST /api/goals/:id/override` — override a gate decision (force proceed
      or stop). Requires admin permission + audit log.
- [x] T22.5 Each intervention emits an event on the SSE stream (G14) + is logged in
      the audit trail.
...
- [x] T23.1 `CapabilityAcquisitionService`: receives `capability_gap` claims from the
      knowledge bus → creates a candidate capability in `swarm_capabilities`.
- [x] T23.2 The next run that needs this capability can use it at `candidate` trust
      level (the planner includes candidate capabilities in `planLoopRun`).
- [x] T23.3 After ≥3 validated successes, the capability is auto-promoted (G1
      `autoPromoteFromEvidence`). After ≥3 failures, auto-deprecated (G1).
- [x] T23.4 A `capability_acquired` event is emitted on the SSE stream (G14) + the
      knowledge bus (G15).
...
- [x] T24.1 `estimateResources(goal)`: estimate CPU/GPU/memory needs from the goal's
      metadata + the capability's learned cost model.
- [x] T24.2 The `ParallelLoopDaemon` scheduler matches goals to available resources
      from `fleetPools()`. A goal that needs GPU waits for GPU availability.
- [x] T24.3 Deferred goals are queued with `reason: 'waiting_for_resources'` and
...
- [x] T25.1 `sanitizeContext(context: string)`: detect injection patterns
      ("ignore previous instructions", "you are now", "system:", "execute:"), strip
      adversarial instructions, add a `[SANITIZED]` tag.
- [x] T25.2 `ContextInjectionService.injectContext` calls `sanitizeContext` on every
      retrieved context before returning it to the maker.
- [x] T25.3 Suspicious context is logged for audit (the sanitization event includes
...
- [x] T26.1 `GET /api/federation/peers` — list known DjimFlo peers.
- [x] T26.2 `POST /api/federation/register` — register a peer (URL + trust level).
- [x] T26.3 `GET /api/federation/capabilities` — list local capabilities; peers can
      subscribe to capability transitions via SSE.
- [x] T26.4 `POST /api/federation/work` — offer work to a peer; the peer accepts or
      rejects based on available capacity.
- [x] T26.5 Peers share claims via the existing `POST /api/knowledge/publish` (G15).
...
- [x] T27.1 Pick a **real, multi-goal, parallel production scenario**: e.g., "add the
...
- [x] T27.2 Run it via the Level-5 swarm: parallel scheduler → 2 concurrent goals →
...
- [x] T27.3 The runs are **green** (`production_passed=true` for both goals), host
      untouched, economically rational, observable in real-time, with distilled memory +
      autonomous capability growth.
- [x] T27.4 OpenSpec closure: archive this change with evidence; publish the ship demo.
