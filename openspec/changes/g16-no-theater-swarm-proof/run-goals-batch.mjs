...
if (!dryRun) {
  console.log(JSON.stringify({
    ...
    auto_spawn_workers: Boolean(batch.policy?.auto_spawn_workers),
    auto_start_loops: Boolean(batch.policy?.auto_start_loops),
    auto_promote_demo_memory: Boolean(batch.policy?.auto_promote_demo_memory),
    requires_live_output: true,
  }));
}
